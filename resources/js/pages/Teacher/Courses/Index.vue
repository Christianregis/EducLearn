    ...
    <div class="flex h-screen overflow-hidden bg-[#F8F7F4] font-sans">
        ...
        <div class="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            <main class="flex-1 overflow-y-auto px-7 pt-7 pb-12 scrollbar-thin scrollbar-thumb-gray-200">
                ...
                <div class="flex items-center gap-3 mb-6 flex-wrap">
                    ...
                    <div class="relative flex-1 min-w-50 max-w-xs">
                        ...
                        <input v-model="search" type="text" placeholder="Rechercher un cours…"
                            class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white text-[13px] font-medium text-gray-800 placeholder-gray-300 outline-none transition-all focus:border-amber-400 focus:shadow-[0_0_0_3px_rgba(251,191,36,0.15)]" />
                    </div>

                    <!-- Category filter -->
                    <div class="relative">
                        <select v-model="filterCategory"
                            class="pl-4 pr-8 py-2.5 rounded-xl border border-gray-200 bg-white text-[12px] font-semibold text-gray-700 outline-none cursor-pointer appearance-none transition-all focus:border-amber-400">
                    ...
                    <!-- View toggle -->
                    <div class="flex items-center bg-white border border-gray-200 rounded-xl p-1 gap-0.5 ml-auto">
                        <button @click="view = 'grid'"
                            class="w-8 h-8 rounded-lg flex items-center justify-center text-[12px] transition-all cursor-pointer border-none"
                            :class="view === 'grid' ? 'bg-amber-400 text-gray-900' : 'bg-transparent text-gray-400 hover:text-gray-700'">
                            <i class="fas fa-grip"></i>
                        </button>
                        <button @click="view = 'list'"
                            class="w-8 h-8 rounded-lg flex items-center justify-center text-[12px] transition-all cursor-pointer border-none"
                            :class="view === 'list' ? 'bg-amber-400 text-gray-900' : 'bg-transparent text-gray-400 hover:text-gray-700'">
                ...
                <div v-if="filteredBooks.length === 0" class="flex flex-col items-center justify-center py-20 gap-4">
                    ...
                    <p class="text-[15px] font-bold text-gray-700">Aucun cours trouvé</p>
                    <p class="text-[13px] text-gray-400">Modifiez vos filtres ou ajoutez un nouveau cours.</p>
                </div>
                ...
                <div v-else-if="view === 'grid'"
                    ...
                    <div v-for="book in filteredBooks" :key="book.id"
                        class="bg-white rounded-2xl border border-gray-100/80 overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 cursor-pointer group"
                        @click="openDetail(book)">
                        <!-- Cover / image zone -->
                        <div class="h-36 flex items-center justify-center relative overflow-hidden"
                            :style="`background: linear-gradient(135deg, ${book.color}18 0%, ${book.color}30 100%)`">
                            <img v-if="book.image" :src="book.image"
                                class="w-full h-full object-cover absolute inset-0" />
                            <i v-else :class="(book.icon || 'fas fa-book') + ' text-[40px]'"
                            ...
                            <span class="absolute top-3 left-3 text-[10px] font-bold px-2.5 py-1 rounded-full"
                                ...
                                <i class="fas fa-file-pdf mr-1 text-[8px]"></i>PDF
                            </span>
                        </div>
                        ...
                        <div class="p-4">
                            <div class="flex items-start justify-between gap-2 mb-2">
                                <p
                                    class="text-[13px] font-bold text-gray-800 leading-snug line-clamp-2 group-hover:text-amber-600 transition-colors flex-1">
                                    {{ book.title }}
                                </p>
                            </div>
                ...
                <!-- ── LIST VIEW ── -->
                <div v-else class="bg-white rounded-2xl border border-gray-100/80 overflow-hidden">
                    <!-- Table header -->
                    <div
                        class="grid grid-cols-12 gap-2 px-5 py-3 bg-gray-50/60 text-[10px] font-bold uppercase tracking-widest text-gray-400 border-b border-gray-50">
                            ...
                            <div class="col-span-5 flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-0.5">
                                        ...
                                        <div class="flex-1 max-w-20 h-1 bg-gray-100 rounded-full overflow-hidden">
        ...
        <Transition name="drawer">
            <div v-if="selectedBook" class="fixed inset-0 z-50 flex justify-end">
                <!-- Backdrop -->
                <div class="absolute inset-0 bg-black/30 backdrop-blur-[2px]" @click="selectedBook = null"></div>
                ...
                <div class="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col z-10">
                    ...
                    <div class="px-6 py-5 flex items-center justify-between border-b border-gray-50"
                        :style="`background: linear-gradient(135deg, ${selectedBook.color}12 0%, ${selectedBook.color}22 100%)`">
                        <div class="flex items-center gap-3">
                            <span class="w-10 h-10 rounded-xl flex items-center justify-center text-[16px]"
                                ...
                                <i :class="selectedBook.icon || 'fas fa-book'"></i>
                            </span>
                            <div>
                                <p class="text-[13px] font-bold text-gray-900">Détail du cours</p>
                                <p class="text-[11px] text-gray-500">{{ selectedBook.category }}</p>
                            </div>
                        </div>
                    ...
                    <div class="h-44 flex items-center justify-center relative"
                        :style="`background: linear-gradient(135deg, ${selectedBook.color}18 0%, ${selectedBook.color}35 100%)`">
                        <img v-if="selectedBook.image" :src="selectedBook.image"
                            class="w-full h-full object-cover absolute inset-0" />
                        <i v-else :class="(selectedBook.icon || 'fas fa-book') + ' text-[56px]'"
                            :style="`color:${selectedBook.color}; opacity:0.5`"></i>
                    </div>

                    <!-- Content -->
                    <div class="p-6 flex flex-col gap-5 flex-1">
                        <div>
                            <h2 class="text-[18px] font-extrabold text-gray-900 leading-snug mb-1">
                                {{ selectedBook.title }}
                            </h2>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div v-for="stat in bookStats(selectedBook)" :key="stat.label"
                                class="bg-gray-50 rounded-xl p-3 flex flex-col gap-1.5">
                        ...
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span
                                class="inline-flex items-center gap-1.5 text-[11px] font-bold px-3 py-1.5 rounded-full bg-red-50 text-red-600">
                                <i class="fas fa-file-pdf text-[9px]"></i>PDF
                            </span>
                        ...
                        <div class="flex items-center gap-3 pt-2 mt-auto">
                            <a :href="selectedBook.file" target="_blank"
                                class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[13px] font-bold transition-all cursor-pointer border-none no-underline"
                                style="background: linear-gradient(135deg, #F4B400 0%, #E09000 100%); color: #0F1117; box-shadow: 0 4px 14px rgba(244,180,0,0.35);">
